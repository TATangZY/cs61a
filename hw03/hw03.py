...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(k, dirc, ret):
        if k == n:
            return ret + dirc
        elif k % 7 == 0 or num_sevens(k) != 0:
            return helper(k + 1, -dirc, ret + dirc)
        else:
            return helper(k + 1, dirc, ret + dirc)
...
def count_change(total):
    ...
    def helper(left, coin):
        if coin == left:
            ...
            return helper(left - coin, coin) + helper(left, coin * 2)
...
def missing_digits(n):
    ...
    n //= 10
    ten_place = n % 10
    gap = one_place - ten_place
    if gap == 0 or gap == 1:
        gap = 0
    else:
        gap -= 1

    return gap + missing_digits(n)
