...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            return scheme_eval(operator.apply_macro(rest, env), env, True)
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        cur = scheme_eval(expressions.first, env, expressions.rest is nil)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        for_cur = formals
        val_cur = vals
        while isinstance(for_cur, Pair):
            child_frame.define(for_cur.first, val_cur.first)
            for_cur = for_cur.rest
            val_cur = val_cur.rest
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        env.define(target.first, do_lambda_form(
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        macro_body = MacroProcedure(target.rest, expressions.rest, env)
        env.define(macro, macro_body)
        return macro
    else:
        if isinstance(target, Pair):
            wrong_target = target.first
        else:
            wrong_target = target

        raise SchemeError('non-symbol: {0}'.format(wrong_target))
    # END Problem 20
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        """
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):  # 如果是尾递归，而且尾递归没有被展开，则直接返回一个 Thunk，不展开递归部分
            return Thunk(expr, env)

        result = Thunk(expr, env)
        # BEGIN
        "*** YOUR CODE HERE ***"
        while isinstance(result, Thunk):   # 如果不是尾递归，则展开递归部分
